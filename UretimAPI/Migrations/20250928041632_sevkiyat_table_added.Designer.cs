// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using UretimAPI.Data;

#nullable disable

namespace UretimAPI.Migrations
{
    [DbContext(typeof(UretimDbContext))]
    [Migration("20250928041632_sevkiyat_table_added")]
    partial class sevkiyat_table_added
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.9")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("UretimAPI.Entities.CycleTime", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("AddedDateTime")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<int>("OperationId")
                        .HasColumnType("int");

                    b.Property<int>("ProductId")
                        .HasColumnType("int");

                    b.Property<int>("Second")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("OperationId");

                    b.HasIndex("ProductId", "OperationId")
                        .IsUnique()
                        .HasDatabaseName("IX_CycleTime_ProductId_OperationId_Unique")
                        .HasFilter("[IsActive] = 1");

                    b.ToTable("CycleTimes");
                });

            modelBuilder.Entity("UretimAPI.Entities.Operation", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("AddedDateTime")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("ShortCode")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.HasKey("Id");

                    b.HasIndex("ShortCode")
                        .IsUnique();

                    b.ToTable("Operations");
                });

            modelBuilder.Entity("UretimAPI.Entities.Order", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("AddedDateTime")
                        .HasColumnType("datetime2");

                    b.Property<int>("Carryover")
                        .HasColumnType("int");

                    b.Property<int>("CompletedQuantity")
                        .HasColumnType("int");

                    b.Property<string>("Customer")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("DocumentNo")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<string>("OrderAddedDateTime")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<int>("OrderCount")
                        .HasColumnType("int");

                    b.Property<string>("ProductCode")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Variants")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("DocumentNo");

                    b.HasIndex("Customer", "IsActive")
                        .HasDatabaseName("IX_Orders_Customer_IsActive");

                    b.HasIndex("OrderAddedDateTime", "IsActive")
                        .HasDatabaseName("IX_Orders_Week_IsActive");

                    b.ToTable("Orders");
                });

            modelBuilder.Entity("UretimAPI.Entities.Packing", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("AddedDateTime")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("Date")
                        .HasColumnType("datetime2");

                    b.Property<string>("ExplodedFrom")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("ExplodingTo")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<string>("ProductCode")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("Quantity")
                        .HasColumnType("int");

                    b.Property<int?>("RelatedWithOrder")
                        .HasColumnType("int");

                    b.Property<string>("Shift")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Supervisor")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("Id");

                    b.HasIndex("Date", "IsActive")
                        .HasDatabaseName("IX_Packing_Date_IsActive");

                    b.HasIndex("ProductCode", "Date", "IsActive")
                        .HasDatabaseName("IX_Packing_ProductCode_Date_IsActive");

                    b.ToTable("Packings");
                });

            modelBuilder.Entity("UretimAPI.Entities.Product", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("AddedDateTime")
                        .HasColumnType("datetime2");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<int>("LastOperationId")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("ProductCode")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("Id");

                    b.HasIndex("LastOperationId");

                    b.HasIndex("ProductCode")
                        .IsUnique();

                    b.HasIndex("Type", "IsActive")
                        .HasDatabaseName("IX_Products_Type_IsActive");

                    b.ToTable("Products");
                });

            modelBuilder.Entity("UretimAPI.Entities.ProductionTrackingForm", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("AddedDateTime")
                        .HasColumnType("datetime2");

                    b.Property<int?>("CastingDefect")
                        .HasColumnType("int");

                    b.Property<int?>("Cleaning")
                        .HasColumnType("int");

                    b.Property<int?>("CycleTime")
                        .HasColumnType("int");

                    b.Property<DateTime>("Date")
                        .HasColumnType("datetime2");

                    b.Property<int?>("DiamondChange")
                        .HasColumnType("int");

                    b.Property<DateTime?>("EndTime")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<string>("Line")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Machine")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<double?>("MachineEfficiency")
                        .HasColumnType("float");

                    b.Property<int?>("MachineFailure")
                        .HasColumnType("int");

                    b.Property<int>("OperationId")
                        .HasColumnType("int");

                    b.Property<double?>("OperatorEfficiency")
                        .HasColumnType("float");

                    b.Property<string>("OperatorName")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int?>("ProcessingDefect")
                        .HasColumnType("int");

                    b.Property<string>("ProductCode")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("Quantity")
                        .HasColumnType("int");

                    b.Property<int?>("RawWaiting")
                        .HasColumnType("int");

                    b.Property<string>("SectionSupervisor")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int?>("SettingMachine")
                        .HasColumnType("int");

                    b.Property<string>("Shift")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("ShiftSupervisor")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime?>("StartTime")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("Date", "IsActive")
                        .HasDatabaseName("IX_PTF_Date_IsActive");

                    b.HasIndex("OperationId", "Date", "IsActive")
                        .HasDatabaseName("IX_PTF_OperationId_Date_IsActive");

                    b.HasIndex("ProductCode", "Date", "IsActive")
                        .HasDatabaseName("IX_PTF_ProductCode_Date_IsActive");

                    b.HasIndex("Shift", "Date", "IsActive")
                        .HasDatabaseName("IX_PTF_Shift_Date_IsActive");

                    b.ToTable("ProductionTrackingForms");
                });

            modelBuilder.Entity("UretimAPI.Entities.Shipment", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<bool>("Abroad")
                        .HasColumnType("bit");

                    b.Property<DateTime>("AddedDateTime")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("Date")
                        .HasColumnType("datetime2");

                    b.Property<int?>("Disk")
                        .HasColumnType("int");

                    b.Property<bool>("Domestic")
                        .HasColumnType("bit");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<int?>("Kampana")
                        .HasColumnType("int");

                    b.Property<int?>("Poyra")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("Date", "IsActive")
                        .HasDatabaseName("IX_Shipment_Date_IsActive");

                    b.ToTable("Shipments");
                });

            modelBuilder.Entity("UretimAPI.Entities.CycleTime", b =>
                {
                    b.HasOne("UretimAPI.Entities.Operation", "Operation")
                        .WithMany("CycleTimes")
                        .HasForeignKey("OperationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("UretimAPI.Entities.Product", "Product")
                        .WithMany("CycleTimes")
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Operation");

                    b.Navigation("Product");
                });

            modelBuilder.Entity("UretimAPI.Entities.Packing", b =>
                {
                    b.HasOne("UretimAPI.Entities.Product", "Product")
                        .WithMany("Packings")
                        .HasForeignKey("ProductCode")
                        .HasPrincipalKey("ProductCode")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Product");
                });

            modelBuilder.Entity("UretimAPI.Entities.Product", b =>
                {
                    b.HasOne("UretimAPI.Entities.Operation", "LastOperation")
                        .WithMany("ProductsWithLastOperation")
                        .HasForeignKey("LastOperationId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("LastOperation");
                });

            modelBuilder.Entity("UretimAPI.Entities.ProductionTrackingForm", b =>
                {
                    b.HasOne("UretimAPI.Entities.Operation", "Operation")
                        .WithMany("ProductionTrackingForms")
                        .HasForeignKey("OperationId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("UretimAPI.Entities.Product", "Product")
                        .WithMany("ProductionTrackingForms")
                        .HasForeignKey("ProductCode")
                        .HasPrincipalKey("ProductCode")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Operation");

                    b.Navigation("Product");
                });

            modelBuilder.Entity("UretimAPI.Entities.Operation", b =>
                {
                    b.Navigation("CycleTimes");

                    b.Navigation("ProductionTrackingForms");

                    b.Navigation("ProductsWithLastOperation");
                });

            modelBuilder.Entity("UretimAPI.Entities.Product", b =>
                {
                    b.Navigation("CycleTimes");

                    b.Navigation("Packings");

                    b.Navigation("ProductionTrackingForms");
                });
#pragma warning restore 612, 618
        }
    }
}
